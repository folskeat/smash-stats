        ...
        <div class="characters">
            <table v-bind:style="{ width: newWidth }">
                ...
                <tr v-for="(character, index) in sortedCharacters" :key="character.csspos">
                    ...
                    <td bgcolor="#83d8eb"><b>{{character.shorthop}}</b></td>
...
export default {
  ...
  methods: {
      ranking(index, shorthop) {
          if (shorthop < this.$options.prevChar) {
              this.$options.prevChar = shorthop;
              this.$options.rank = index;
              return index;
          }
          else if (shorthop === this.$options.prevChar) {
