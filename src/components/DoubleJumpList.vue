            ...
            <table v-bind:style="{ width: newWidth }">
                ...
                <thead>
                    <tr>
                        ...
                        <th bgcolor="#83d8eb">Double Jump Height</th>
                    </tr>
                </thead>
                <tr v-for="(character, index) in sortedCharacters" :key="character.csspos">
                    <td>{{ranking((index + 1), character.doublejump)}}</td>
                    ...
                    <td bgcolor="#83d8eb"><b>{{character.doublejump}}</b></td>
                </tr>
            </table>
        </div>
    </div>
...
export default {
  ...
  methods: {
      ranking(index, doublejump) {
          if (doublejump < this.$options.prevChar) {
              this.$options.prevChar = doublejump;
              this.$options.rank = index;
              return index;
          }
          else if (doublejump === this.$options.prevChar) {
